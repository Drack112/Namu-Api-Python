...
async def test_returns_none_when_below_min_samples(rows):
...
async def test_avg_rating_and_total_feedbacks():
    rows = [
...
async def test_invalid_activities_produce_empty_preferred(rows):
    session = _session_with_rows(rows)
    ctx = await build_feedback_context(1, session)
    assert ctx is not None
    assert ctx["preferred_categories"] == []
